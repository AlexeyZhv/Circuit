...
public:
    ...
    void print() {
        std::cout << "This is a circuit with following elements:" << '\n' << std::endl;
        for (size_t i = 0; i != resistors.size(); ++i) {
            std::cout << "Resistor: " << resistors[i]->getName() << ", Current: " << resistors[i]->get_cur() << ", Voltage: " << resistors[i]->get_vol() << std::endl;
        }
        for (size_t i = 0; i != wires.size(); ++i) {
            std::cout << "Wire: " << wires[i]->getName() << ", Current: " << wires[i]->get_cur() << ", Voltage: " << wires[i]->get_vol() << std::endl;
        }
    }
...
void createCircuitFromData(std::vector<std::string>& nodes1, std::vector<std::string>& nodes2,
    ...
    for (size_t i = 0; i < types.size(); ++i) {
        if (types[i] == "Resistor") {
            ...
            Bar* new_inductor = new Inductor(values[i], node_map[nodes1[i]], node_map[nodes2[i]]);
